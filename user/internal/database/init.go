...
func Initialize(dbVarName, dbPort string) (*gorm.DB, error) {
	...
	if databaseType == "sqlite" {
		...
		// dsn := fmt.Sprintf("host=db_user user=%s password=%s dbname=%s port=%s sslmode=disable TimeZone=Europe/Kyiv", user, pass, dbname, dbPort)
		// NOTE inside docker always 5432
		dsn := fmt.Sprintf("host=db_user user=%s password=%s dbname=%s port=5432 sslmode=disable TimeZone=Europe/Kyiv", user, pass, dbname)
